...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    foreach ($campos as $campo) {
        $stmt = $pdo->prepare("REPLACE INTO site_content (section, field_name, content) VALUES ('header', ?, ?)");
...
function get_value($campo) {
    global $pdo;
    $stmt = $pdo->prepare("SELECT content FROM site_content WHERE section = 'header' AND field_name = ?");
    $stmt->execute([$campo]);
...
<body>
    <div class="container mt-5">
        <h2>Editar Títulos da Página</h2>
        ...
        <form method="POST">
            <div class="mb-3">
                ...
                <input type="text" name="title" value="<?= htmlspecialchars(get_value('title')) ?>" class="form-control" required>
            </div>
            <div class="mb-3">
